...
This environment includes **Flox-aware templates** that make Goose automatically use Flox for environment management in your projects.
...
```bash
# Copy Goose profile configuration
mkdir -p ~/projects/my-app/.goose
cp $FLOX_ENV_TEMPLATES/.goose/profiles.yaml ~/projects/my-app/.goose/

# Or copy the general instructions file
cp $FLOX_ENV_TEMPLATES/GOOSE.md ~/projects/my-app/
...
The templates instruct Goose to:
...
5. **Profile-based instructions** embedded in `.goose/profiles.yaml`
...
# With default profile (includes Flox awareness)
goose session start

You: I need to set up GPU acceleration with CUDA for this ML project

Goose: [Detects CUDA task]
Goose: [Runs: ./fetch-flox.sh cuda]
Goose: [Reads FLOX.md for CUDA guidance]
Goose: [Applies Flox CUDA patterns]
Goose: Let me help you set up CUDA using Flox...
```

**Using Ollama profile with Flox:**

The template includes an `ollama-local` profile for privacy-focused local development:

```bash
# Use local model with Flox awareness
goose session start --profile ollama-local

You: Package this Python application

Goose: [Uses local Qwen model]
Goose: [Fetches Flox packaging context]
Goose: [Applies Flox patterns locally]
```

**Customization:**

Edit `.goose/profiles.yaml` to:
- Add project-specific instructions to the `instructions` field
- Create custom profiles for different workflows
- Configure model preferences while maintaining Flox awareness

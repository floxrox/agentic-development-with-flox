...
This environment includes **Flox-aware templates** that make ECA automatically use Flox for environment management in your projects.
...
```bash
# Copy ECA rules
mkdir -p ~/projects/my-app/.eca/rules
cp $FLOX_ENV_TEMPLATES/.eca/rules/flox-environment.md ~/projects/my-app/.eca/rules/

# Copy universal AGENTS.md (ECA supports this standard)
# Note: Get AGENTS.md from spec-kit environment if needed
...
5. **Auto-load rules** based on globs for relevant files
...
# ECA runs as a background service
curl http://localhost:5173/api/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "I need to build and package this Go application"}'

ECA: [Detects packaging task]
ECA: [Runs: ./fetch-flox.sh packaging]
ECA: [Reads FLOX.md for packaging guidance]
ECA: [Applies Flox packaging patterns]
ECA: Let me help you package this Go app using Flox...
```

**Integration with ECA's service model:**

ECA's background daemon works seamlessly with Flox:
- Flox manages all dependencies including ECA
- Services defined in `.flox/env/manifest.toml` integrate with ECA
- Both support declarative, reproducible configuration
- AGENTS.md provides universal guidance across projects

**Customization:**

Add your project-specific rules in `.eca/rules/` with description and globs metadata.
